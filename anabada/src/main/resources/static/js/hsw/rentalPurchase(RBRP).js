$(document).ready(function(){
	...
	*/	
	function pricecal() {
		let edate = new Date($('.rental_edate').val()).getTime()
		let sdate = new Date($('.rental_sdate').val()).getTime()
		let count = edate - sdate;
			
		let countday = count / (1000 * 60 * 60 * 24);
		let total = $('.rental_price_day').val() * countday;
		if($.isNumeric(total) && total > 0){
			$('.showrental_price').val(comma(total)+'원');
			$('.rental_price').val(total);
			$('.showmoney').val($('.showrental_price').val());	
			$('.money').val($('.rental_price').val());	

	}
		else{
			$('.showrental_price').val(0);
			$('.rental_price').val(0);
			$('.showmoney').val($('.showrental_price').val()+'원');	
			$('.money').val($('.rental_price').val());	
		}
		
		if($('.rental_price').val() <= 0){
